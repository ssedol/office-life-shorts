...
>
> **개정 이력**
> - v1.0 (최초) — 아래 "v1.0 원안" 참고.
> - **v1.1 (2026-09-21)** — 역할 분담 개정. ChatGPT의 역할을 **이미지 생성으로 한정**하고,
>   주제·대본·8씬 구성·`scene-plan.json`을 로컬 작업자(Claude)가 맡는다. DEC-016 참고.
>   자동 YouTube 업로드를 범위에 넣는다. DEC-017 참고.
>
> 핵심 전제 (v1.1):
> - **주제·제목·대본·8씬 구성·`scene-plan.json`은 로컬 작업자(Claude)가 만든다.**
> - **ChatGPT가 담당하는 것은 씬 이미지 8장 생성뿐이다.**
>   로컬에서 이미지를 만들지 못해 그 공정만 위탁하는 것이지, 기획을 넘긴 것이 아니다.
> - **씬별 STILL / I2V 판단도 로컬 작업자가 한다.** 렌더 파이프라인은 이를 재판단하지 않는다.
> - 로컬 프로젝트는 기획부터 렌더·업로드까지 이미지 생성을 뺀 전 공정을 처리한다.
>
> v1.0 원안 (초과 — 더 이상 따르지 않는다):
...
> **v1.1 개정 (2026-09-21, DEC-018).** 원안에 "훈계조 금지"와 "팁 중심"이 나란히 있어
> 서로 반대 방향을 가리켰다. 1~3편이 모두 "이렇게 하세요"로 끝나는 처세술 영상이 된 원인이다.
> 우선순위를 명시한다.

1. **공감이 중심이다.** 시청자가 "아 맞아, 나도 저래"라고 느끼는 게 목표다.
   팁은 곁들이는 것이지 중심이 아니다.
2. **내 실패담으로 푼다.** 일반론("이런 상황에선 이렇게")이 아니라
   내가 겪은 일을 이야기한다. 공감은 논평이 아니라 장면에서 걸린다.
3. **훈계로 끝내지 않는다.** "이렇게 하세요"로 닫으면 대화가 끊긴다.
   해결을 말하되 "나도 아직 완벽하진 않다"를 남겨 시청자가 답할 자리를 만든다.
4. 가볍고 재밌게. **웃픈 구석**이 있어야 한다.
5. "회사 다녀본 사람은 바로 알아듣는" 느낌.
6. 역할극보다 **1인 내레이션 중심**.

소재는 위 "채널 주제"를 고루 돈다. 상사 얘기만 반복하지 않는다
— 1~3편이 모두 "상사가 일을 시킨다"였다. 동료·눈치·회식·점심·퇴근도 쓴다.
...
## 7.1 로컬 작업자 (Claude) — 기획

> v1.1 개정 (DEC-016). v1.0에서는 이 항목이 "외부 AI / 기획 AI"였다.
...
- 씬별 상황 설명
- 씬별 핵심 대사
- `scene-plan.json` / `script.txt` 작성
- 씬별 STILL / I2V 판단
- ChatGPT에 넘길 `image-prompts.md` 작성
- 업로드 메타데이터(`upload.json`) — 제목 / 설명 / 태그 / 첫 댓글

## 7.2 ChatGPT

담당은 **이미지 생성뿐이다** (v1.1 개정, DEC-016).
로컬에서 이미지를 생성할 수 없어 이 공정만 위탁한다.
...
판단 주체는 **로컬 작업자(Claude)**다 (v1.1 개정, DEC-016).
판단 결과는 `scene-plan.json`의 `type` 필드에 적는다.

**렌더 파이프라인은 이를 재판단하지 않는다.** `scene-plan.json`을 그대로 따른다.

> v1.0에서는 판단 주체가 ChatGPT였다. 아래 기준 자체는 그대로 쓴다.
...
- **DEC-008** 이미지 8장은 ChatGPT 생성. *(유효)*
- **DEC-009** ~~STILL/I2V 추천도 ChatGPT 담당.~~ → **DEC-016으로 대체.**
- **DEC-010** ~~로컬은 이미지 이후 공정만 담당.~~ → **DEC-016으로 대체.**
- **DEC-011** I2V는 LTX 2.5.
- **DEC-012** I2V는 기본 2~3씬.
- **DEC-013** TTS는 무료 한국어 음성부터.
- **DEC-014** Preview → 검수 → Final 유지.
- **DEC-015** 로컬 I2I는 MVP에서 제외.
- **DEC-016** *(2026-09-21)* **ChatGPT의 역할은 이미지 생성으로 한정한다.**
  주제·제목·대본·8씬 구성·`scene-plan.json`·`script.txt`·STILL/I2V 판단·
  `image-prompts.md`·업로드 메타데이터는 **모두 로컬 작업자(Claude)가 만든다.**
  이미지만 위탁하는 이유는 로컬에서 이미지를 생성할 수 없기 때문이지
  기획을 외부에 넘기기로 한 것이 아니다. DEC-009 / DEC-010을 대체한다.
- **DEC-018** *(2026-09-21)* **콘텐츠는 공감이 중심이고 팁은 곁들이는 것이다.**
  원안 §1.2의 "훈계조 금지"와 "팁 중심"이 충돌해 1~3편이 모두 처세술 영상이 됐다.
  공감 > 팁으로 우선순위를 못 박고, 마무리를 "이렇게 하세요"로 닫지 않는다.
  소재도 상사에 몰리지 않게 고루 돈다.
...
2. **렌더 파이프라인은 STILL/I2V를 재판단하지 않는다** — `scene-plan.json`을 그대로 따른다.
   (판단 자체는 로컬 작업자가 한다. DEC-016)
3. 씬 수는 8개로 유지한다.
4. `scene-plan.json`이 렌더의 기준이다. (v1.0의 "ChatGPT가 제공한"은 DEC-016으로 무효)
5. I2V 실패 시 전체 실패가 아니라 STILL fallback.
6. Preview 검수 단계를 유지한다.
7. ~~MVP에서 자동 YouTube 업로드까지 확장하지 않는다.~~ → **DEC-017로 대체.**
...
```text
로컬 작업자 (Claude)
주제 / 제목 / 대본 / 8씬 / scene-plan.json / STILL·I2V 판단
        ↓
ChatGPT
이미지 8장  ← 위탁하는 공정은 여기뿐이다
        ↓
로컬
STILL / LTX 2.5 I2V
        ↓
